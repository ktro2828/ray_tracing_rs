...
fn sample_ray(path: PathBuf) {
    ...
    let image_height = if (image_width as f64 / aspect_ratio) < 0.0 {1} else {(image_width as f64 / aspect_ratio) as i32};

    // camera
    let focal_length = 1.0;
    let viewport_height = 1.0;
    let viewport_width = viewport_height * (image_width / image_height) as f64;
    let camera_center = Vec3::from(0.0, 0.0, 0.0);

    // calculate the vectors across the horizontal and down the vertical viewport edges.
    let viewport_u = Vec3::from(viewport_width as f64, 0.0, 0.0);
    let viewport_v = Vec3::from(0.0, -viewport_height as f64, 0.0);
    ...
    let viewport_upper_left = camera_center - Vec3::from(0.0, 0.0, focal_length) - (viewport_u + viewport_v) * 0.5;
...
fn get_ray_color(ray: &Ray) -> Color {
    ...
    if t > 0.0 {
        let n = (ray.at(t) - Vec3::from(0.0, 0.0, -1.0)).as_unit();
